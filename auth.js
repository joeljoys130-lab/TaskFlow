...
const isEJConfigured = () => { const c = getEJConfig(); return !!(c && c.publicKey && c.serviceId && c.templateId); };
...
const avatarInitials = n => n.trim().split(' ').map(w => w[0]).join('').toUpperCase().slice(0, 2);
function avatarColor(email) {
    const cols = ['#7c3aed', '#3b82f6', '#06b6d4', '#10b981', '#f59e0b', '#ef4444', '#ec4899', '#8b5cf6'];
...
function checkOtpFilled() {
    otpVerifyBtn.disabled = [...otpInputs].map(i => i.value).join('').length < 6;
}
...
function startResendCountdown(sec = 60) {
    ...
    resendTimer = setInterval(() => {
        t--;
        if (t <= 0) { clearInterval(resendTimer); otpResend.disabled = false; otpResend.textContent = 'Resend Code'; }
    ...
    try {
        if (isEJConfigured()) {
            ...
            setTimeout(() => {
                ...
                notifyDevice("TaskFlow Security Code", `Your verification code is: ${otp}`);
            }, 800);
        }
        startResendCountdown(60);
    } catch (err) {
        console.error('EmailJS error:', err);
        setOtpStatus('failed', '', err?.text || err?.message || 'Unknown configuration error');
...
async function doSignup() {
    ...
    if (name.length < 2) return showErr(signupErr, 'Name must be at least 2 characters.');
    if (!email) return showErr(signupErr, 'Please enter your email.');
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) return showErr(signupErr, 'Please enter a valid email address.');
    if (!pass) return showErr(signupErr, 'Please enter a password.');
    if (measureStrength(pass) < 2) return showErr(signupErr, 'Password too weak — use 8+ characters with numbers.');
...
async function verifyOtp() {
    let pending;
    try { pending = JSON.parse(localStorage.getItem(PENDING_KEY)); } catch { pending = null; }
    if (!pending) return showErr(otpErr, 'Session expired. Please sign up again.');
    ...
    if (isDemo && entered !== pending.otp) {
        ...
        if (entered !== pending.otp) return showErr(otpErr, 'Invalid verification code.');
        if (Date.now() - pending.otpTs > 10 * 60 * 1000) return showErr(otpErr, 'Code expired. Please resend.');
    }
    ...
    try {
        ...
        const userCredential = await firebase.auth().createUserWithEmailAndPassword(pending.email, pending.plainPass);
...
setupSaveBtn?.addEventListener('click', () => {
    ...
    if (ret === 'signup') {
        ...
        try {
            const saved = JSON.parse(localStorage.getItem('taskflow_pending_signup') || '{}');
            if (saved.name) signupName.value = saved.name;
            if (saved.email) signupEmail.value = saved.email;
            if (saved.pass) { signupPass.value = saved.pass; signupConfirm.value = saved.pass; }
        ...
        if (session) { authOverlay.classList.add('hidden'); mainApp.classList.remove('hidden'); }
        else {
            showScreen(screenLogin);
...
setupTestBtn?.addEventListener('click', async () => {
    ...
    try {
        ...
        await emailjs.send(cfg.serviceId, cfg.templateId, {
            to_email: testEmail, to_name: 'Test User', otp_code: '123456',
            app_name: 'TaskFlow', expiry: '10 minutes'
        });
        ...
        showErr(setupErr, `❌ Send failed: ${err?.text || err?.message || 'Check your credentials.'}`);
    } finally {
